...
path = '/home/unshakensoul/Documents/Projects and Notes/Projects/Architect AI/code-indexer-extension/webview/src/components/GraphCanvas.tsx'

with open(path, 'r', encoding='utf-8') as f:
    content = f.read()

target_start_text = '    // Build all nodes and edges from graph data (only when data changes)'
target_start = content.find(target_start_text)

target_end_text = '    }, [graphData, currentMode, collapsedNodes, toggleNodeCollapse, architectureSkeleton, functionTrace, selectedDomain, sortBy, maxDepth]);'
target_end = content.find(target_end_text, target_start) + len(target_end_text)

if target_start == -1 or target_end < target_start:
    print("Could not find target range.")
    exit(1)

original_block = content[target_start:target_end]

replacement = """    // Stable callback ref to avoid re-running memo when function changes
...
print("Successfully rewrote component logic in GraphCanvas!")
